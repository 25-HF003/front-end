...
function SecondHome() {
  ...
  return(
      <div ref={targetRef} className="gap-6 mt-6 bg-white flex flex-col justify-center items-center text-white-100 text-[64px]" style={{ minHeight: "calc(100vh + 90px)" }}>
        <Link to="/detection" className="flex items-center gap-6">
          <span>딥페이크 탐지</span>
          <img src="/arrow1.svg" alt="화살표" />
          <p>Now</p>
        </Link>
        <Link to="/adversarial-noise" className="flex items-center gap-6">
          <span>적대적 노이즈 삽입</span>
          <img src="/arrow2.svg" alt="화살표" />
          <p>Now</p>
        </Link>
        <Link to="/watermark-insert" className="flex items-center gap-6">
